...
namespace DataServices.Repositories
{
    ...
    {
        public ASSINANTE CheckExist(ASSINANTE assinante)
        {
            IQueryable<ASSINANTE> query = Db.ASSINANTE;
            if (assinante.ASSI_IN_PESSOA == 1)
            {
                query = query.Where(p => p.ASSI_NR_CPF == assinante.ASSI_NR_CPF);
            }
            else
            {
                query = query.Where(p => p.ASSI_NR_CNPJ == assinante.ASSI_NR_CNPJ);
            }
